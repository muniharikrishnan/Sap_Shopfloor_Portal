sap.ui.define([
    ...
    return Controller.extend("SHOPFLOOR.controller.PlannedOrders", {
        ...
        _loadData: function () {
            ...
            // OData call to fetch planned orders
            var sUrl = "/sap/opu/odata/SAP/Z48_PP_PORTAL_SRV/PPPLANNEDORDERSet?$filter=Plwrk eq '" + sPlant + "'&$format=json";
            jQuery.ajax({
                url: sUrl,
                type: "GET",
                dataType: "json",
        ...
        _applyFilters: function (sQuery) {
            ...
            var aFilteredData = oCurrentData.filter(function (oItem) {
                // Search by planned order number
                if (oItem.Plnum && oItem.Plnum.toString().includes(sQuery)) {
                    return true;
                }
                ...
                if (oItem.Dispo && oItem.Dispo.toString().toLowerCase().includes(sQuery.toLowerCase())) {
                    return true;
                }
