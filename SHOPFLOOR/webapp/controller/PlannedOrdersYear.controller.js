sap.ui.define([
    ...
    return Controller.extend("SHOPFLOOR.controller.PlannedOrdersYear", {
        onInit: function () {
        ...
        _loadData: function () {
            ...
            // OData call to fetch year-wise planned orders
           
             var sUrl = "/sap/opu/odata/SAP/Z48_PP_PORTAL_SRV/pp_planorder_yearSet?$filter=Plwrk eq '" + sPlant + "'&$format=json";
            jQuery.ajax({
                url: sUrl,
                type: "GET",
                dataType: "json",
        ...
        _applyFilters: function (sQuery) {
            ...
            var aFilteredData = oCurrentData.filter(function (oItem) {
                // Search by year
                if (oItem.Zyear && oItem.Zyear.toString().includes(sQuery)) {
                    return true;
                }
        ...
        _applyDateRangeFilter: function () {
            ...
            var aFilteredData = oCurrentData.filter(function (oItem) {
                ...
                if (oItem.Psttr) {
                    var sStartMatch = oItem.Psttr.match(/\/Date\((\d+)\)\//);
                ...
                if (oItem.Pedtr) {
                    var sEndMatch = oItem.Pedtr.match(/\/Date\((\d+)\)\//);
